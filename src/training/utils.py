...
def get_kl_loss(teacher_logits, student_logits, student_labels, teacher_labels, temperature, distill_topk=None):
    ...
    student_mask = (student_labels != -
                    100).unsqueeze(-1).expand_as(student_logits)
    student_logits_selected = torch.masked_select(
        student_logits, student_mask).view(-1, vocab_size)

    teacher_mask = (teacher_labels != -
                    100).unsqueeze(-1).expand_as(teacher_logits)
    teacher_logits_selected = torch.masked_select(
        teacher_logits, teacher_mask).view(-1, vocab_size)
...
def save_combined_model_with_accelerate(accelerator, combined_model, model_tokenizer, retriever_tokenizer, output_dir):
    ...
    if accelerator.is_main_process:
        ...
        model_state = {k[6:]: v for k,
                       v in full_state.items() if k.startswith("model.")}
        retriever_state = {
            k[10:]: v for k, v in full_state.items() if k.startswith("retriever.")}
...
def get_retrieval_embeds_query_aware(retriever, input_query_ids, query_attention_mask, input_ids, attention_mask=None):
    ...
    """
    embeds = retriever.get_query_qware_embedding(
...
def get_retrieval_nnembd_embeds(retriever, model, input_ids, attention_mask=None):
    ...
    retriever_embeds = retriever_embeds.view(-1, retriever_embeds.shape[-1])
    nnemb = torch.mean(model.model.embed_tokens(input_ids),
                       dim=1)  # Shape: [batch_size, 4096]
...
def get_nn_embeds(model, input_ids, attention_mask=None):
    ...
    if "qwen" in model_name.lower():
        nnembeds = torch.mean(original_model.get_embed_tokens(
            input_ids), dim=1)  # Shape: [batch_size, 4096]
    else:
        nnembeds = torch.mean(
            original_model.model.embed_tokens(input_ids), dim=1)
...
def find_matched_index(main_seq, sub_seq):
    ...
    assert len(sub_seq) > 0 and len(
        main_seq) > 0, f"the input should not be empty, however {sub_seq=}\n {main_seq=}"
