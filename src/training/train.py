...
def parse_args():
    ...
    parser.add_argument(
        "--chat_format",
        choices=['mistral','tulu','mixtral','qwen','yi','gemma','llama','deepseek'],
    )
    parser.add_argument(
        "--max_train_samples",
        type=int,
...
def collator(        
    ...
    placeholder_input_ids,placeholder_attention_mask,placeholder_labels = padding(
        ...
        labels=[x['placeholder_labels'] for x in samples] if 'placeholder_labels' in samples[0].keys() else None,
...
def main():
    ...
    init_process_group_kwargs = InitProcessGroupKwargs(timeout=timedelta(seconds=108000))  # 30 hrs 
    accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps,kwargs_handlers=[init_process_group_kwargs])
    ...
    if args.retriever_name_or_path is not None:
        if args.retriever_name_or_path.lower() == args.model_name_or_path.lower():
            ...
            if 'bge-small' in args.retriever_name_or_path.lower():
                retriever = BGE_SMALL.from_pretrained(args.retriever_name_or_path,torch_dtype = torch.bfloat16)
                ...
                retriever = BGE.from_pretrained(args.retriever_name_or_path,torch_dtype = torch.bfloat16)
                retriever_tokenizer = AutoTokenizer.from_pretrained(args.retriever_name_or_path)
                retrieval_embed_length = retriever.get_embed_length()
                retriever_hidden_size = retriever.get_embed_dim()
            elif 'qwen2' in args.retriever_name_or_path.lower():
                retriever = Qwen2.from_pretrained(args.retriever_name_or_path,torch_dtype = torch.bfloat16)
                retriever_tokenizer = AutoTokenizer.from_pretrained(args.retriever_name_or_path)
                retrieval_embed_length = retriever.get_embed_length()
                retriever_hidden_size = retriever.get_embed_dim()
    ...
    args.output_dir = args.output_dir+args.task_type+"/"+ args.chat_format + "/" + str(time.strftime("%Y%m%d_%H%M%S")) + "/"
    ...
    num_added_tokens += tokenizer.add_tokens([AddedToken(PLACEHOLDER_TOKEN,lstrip=False,rstrip=False)])
        ...
        if args.task_type == 'finetune':
            ...
            if args.alpha_kl is not None and args.alpha_kl > 0.0:
                lm_datasets['train'] = lm_datasets['train'].filter(
                    lambda example: 
                    (example['labels']!=-100).sum() == (example['placeholder_labels']!=-100).sum()
                )
    ...
    retriever_layers_to_update = ["layers." + str(i) for i in range(retriever.config.num_hidden_layers - args.update_retriever_layers, retriever.config.num_hidden_layers)]
    for n,p in combined_model.retriever.named_parameters():
        p.requires_grad = False
        for layer in retriever_layers_to_update:
            if layer in n:p.requires_grad = True
        
    optimizer = torch.optim.AdamW([p for p in combined_model.model.parameters() if p.requires_grad]+[p for p in combined_model.retriever.parameters() if p.requires_grad],lr=args.learning_rate)
    ...
    logger.info(f"  Trainable Parameters = {sum(p.numel() for p in combined_model.parameters() if p.requires_grad)/(10**6):.2f} M") ## not applicable for deepspeed
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for batch in active_dataloader:
            if save_one_sample:
                ...
                accelerator.print(tokenizer.decode(batch['placeholder_input_ids'][0]))
            ...
            with accelerator.accumulate(combined_model):
                outputs = combined_model(
                    ...
                    retriever_query_input_ids = batch.get('retriever_query_input_ids',None),
                    retriever_query_attention_mask_for_model = batch.get('retriever_query_attention_mask_for_model',None),
                ...
                if accelerator.sync_gradients and args.clip_grad_norm > 0:
                    accelerator.clip_grad_norm_(combined_model.parameters(), args.clip_grad_norm)
                optimizer.step()
                optimizer.zero_grad()
                lr_scheduler.step()       
                    ...
                    if completed_steps % checkpointing_steps == 0:
                        ...
                        if dev_dataloader is not None:
                            ...
                            if args.task_type == 'pretrain':
                                ppl = validate_during_pretrain(combined_model.model,dev_dataloader,accelerator,vocab_size,combined_model.retriever)
            ...
            if accelerator.is_local_main_process:
                if save_count < 10:
                    ...
                    pickle.dump(
                        save_dict,
                        open(os.path.join(os.path.dirname(args.output_dir),"sample_data.pkl"),'ab'),
                    )
                save_count += 1
    ...
    save_combined_model_with_accelerate(accelerator, combined_model, tokenizer, retriever_tokenizer,output_dir)
