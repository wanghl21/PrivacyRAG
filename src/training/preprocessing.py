...
def _concat_messages_mixtral(messages, tokenizer):
    """
    Format messages according to Mixtral chat format.
    Delegates to the Mistral format as they use the same formatting.
    ...
    """
    ## Mixtral Chat Format uses the same format as Mistral
...
def _encode_chat_format(
    ...
    example_text = tokenizer.apply_chat_template(messages, tokenize=False).strip()
    ...
    for message_idx, message in enumerate(messages):
        if message["role"] != "assistant":
            ...
            messages_so_far = tokenizer.apply_chat_template(messages[:message_idx+1],tokenize=False)         
...
def encode_with_chat_format_pretrain(
    ...
    if use_sentence_split or use_split:
        if use_sentence_split:
            documents = split_background_sentence(document,tokenizer,total_max_len=max_seq_length,single_max_len=retrieval_context_single_max_len) 
        elif use_split:
            documents = split_background(document,tokenizer,total_max_len=max_seq_length,single_max_len=retrieval_context_single_max_len) 

        if len(documents)==0:
            placeholder_token = " ".join([PLACEHOLDER_TOKEN]*retrieval_embed_length)
            documents = document
        else:
            placeholder_token = " ".join([PLACEHOLDER_TOKEN]*retrieval_embed_length*len(documents))
        instruction = random.choice(ParaphraseInstructions).format_map(dict(placeholder_token=placeholder_token))
        messages = [
...
#             tokenizer=tokenizer,
#             max_seq_length=args.max_seq_length,=1024
#             retrieval_embed_length=retrieval_embed_length,=1
def encode_with_chat_format_finetune(
    ...
    if use_rag_tuning and use_retriever_embed:
        if use_split:
            if use_sentence_split:
                sharded_background = split_background_sentence(background,retriever_tokenizer,total_max_len=max_seq_length,single_max_len=180)  
            else:
                sharded_background = split_background(background,retriever_tokenizer,total_max_len=max_seq_length,single_max_len=retrieval_context_single_max_len)
    ...
    if use_rag_tuning:
        ...
        placeholder_tokens = " ".join([PLACEHOLDER_TOKEN]*retrieval_embed_length* num_split)
...
def encode_with_qa_format(
    ...
    if use_rag_tuning:
        ...
        prompt_background = " ".join([PLACEHOLDER_TOKEN]*retrieval_embed_length)
        ...
        ret["placeholder_input_ids"] = input_ids.flatten()
        ret['placeholder_labels'] = labels.flatten()
...
def encode_with_completion_format_pretrain(example,tokenizer,max_seq_length,retrieval_embed_length,placeholder_token_id):
    document = example['text'].strip()
    ...
    prompt = prompt.format_map(dict(placeholder_token=placeholder_token,document=_document))
    ...
    labels = input_ids.clone()
    labels[labels==placeholder_token_id] = -100
    ...
    return {
        "placeholder_input_ids":input_ids.flatten(),
        "placeholder_labels":labels.flatten(),
        "retriever_input_text":[document],
    }
...
def encode_with_completion_format_finetune(
    ...
    if use_rag_tuning:
        ...
        for idx,prompt_background in enumerate([
            " ".join([PLACEHOLDER_TOKEN]*retrieval_embed_length* num_split),
            background,
        ]):
    ...
    #     return {
    #         "placeholder_input_ids":tokenized_prompt.input_ids.flatten(),
